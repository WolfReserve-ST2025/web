...
const CartModal = ({ open, draftOrder, onClose, onSubmit }: CartModalProps) => {
    if (!open) return null;
                    ...
                    draftOrder.foods.length === 0 ? (
                        ...
                        <ul className="space-y-4">
                            {draftOrder.foods.map((item, idx) => (
                                <li key={item._id || idx} className="flex justify-between items-center border-b pb-2">
                                    <span>{item.name}</span>
